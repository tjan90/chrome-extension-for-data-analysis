...
chrome.storage.local.get(['links','id','counter','pr_price'],function(result) {
    ...
    chrome.storage.local.get('visit_links1', function (res) {

        //alert(res.visit_links1);
        if (res.visit_links1 == undefined) {
            windowOpen(result.links, result.id, result.pr_price)
            chrome.storage.local.set({'visit_links1': result.links});
                }
...
//        var product_price = document.getElementById(parent_tag_id).innerText;
                ...
                //var sp_price = product_price_cl[product].innerText;
