...
*/
console.log('From Popup.js file');
var executionAPIExample = (function() {

	var SCRIPT_ID='1qDRS53QG4LdbmiXezef0O7T2se42Dltoh5J9H1rv6eGudKqzCTlBlsn9'; // Apps Script script id
	var STATE_START=1;
	...
	var signin_button, xhr_button, revoke_button, exec_info_div, exec_data, exec_result, retrieve, analyze;
	...
	/**
	* 		Data Retrieved from Google Sheet into extension
	* */

	function retrieveDataFromExecutionAPI() {
		disableButton(xhr_button);
		xhr_button.className = 'loading';
		getAuthToken({
			'interactive': false,
			'callback': retrieveDataToExecutionAPICallback,
		});
	}
	...
	function sendDataToExecutionAPICallback(token) {
		...
		//$(document).load('https://script.google.com/macros/s/AKfycbzEI2v9XbRzDdy6DiTtSpLU9hwQwPN0CCrJ-QaoYkXNz72q14Q/exec')
		console.log(JSON.parse(exec_data.value));
	}
	/**
	* 	Data Retrieved from Google Sheet into extension
	* */
	function retrieveDataToExecutionAPICallback(token) {
		sampleSupport.log('Retrieving data to Execution API script');
		post({	'url': 'https://script.googleapis.com/v1/scripts/' + SCRIPT_ID + ':run',
			'callback': executionAPIResponseUponRetrieval,
			'token': token,
			'request': {'function': 'retrieveData',
				'parameters': {'data' : JSON.parse(exec_data.value)},

			}//end_parameters
		});//end_request
        var jsonData = exec_data.value;
        exec_result.innerHTML = JSON.parse(jsonData);
	...
	function executionAPIResponseUponRetrieval(response){
		if(response.done == true){
			sampleSupport.log('Data Retrieval Successfull')
		}else{
			sampleSupport.log('Error During Retrieval')
		}
		//console.log(response.response.result.length+'\n'+response.response.result[0].length);

		exec_data.value = JSON.stringify(response.response.result);
		...
		onload: function () {
			try {
				...
				retrieve.addEventListener('click', function(){
					...
					retrieveDataFromExecutionAPI();

				});
				analyze = document.querySelector('#analyze');

				chrome.storage.local.get(null, function (result) {
					var url = '{"depth":' + result.depth + ', "array":"' + result.array + '", "links":"' + result.links + '", "price":"' + result.price + '"}';
					exec_data.value = url;
				});
